...
export async function PracticeAreas() {
  ...
  const areas = await getAllPracticeAreas(locale);
  return (
    <section className="bg-ivory px-5 py-16 md:px-[72px] md:py-24">
      <SectionHead
        eyebrow={t("practiceWhatWeDo")}
        title={t("practiceTitle")}
        viewAllHref="/practice-areas"
        viewAllLabel={t("practiceViewAll")}
      />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
        {areas.map((area) => (
          ...
          >
            ...
            <PracticeIcon
              name={area.icon_name as IconName}
              className="w-6 h-6 text-forest mb-3 relative z-10"
            />
            <div className="font-serif text-[11px] text-gold mb-1 relative z-10">
              {formatNum(area.display_order)}
            </div>
